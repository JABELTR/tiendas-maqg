...
const CruceAddi = () => {
  ...
  const subir = async () => {
    ...
    try {
      ...
      const { data } = await axios.post("/cruce-addi/upload", form, {
...
const exportarExcel = async () => {
  ...
  try {
    const res = await axios.post("/cruce-addi/export-excel", formData, {
      headers: { "Content-Type": "multipart/form-data" },
      responseType: "blob",
    });  
    ...
    link.href = url;
    link.setAttribute("download", "cruce_addi.xlsx");
  ...
  return (
    <Box p={2}>
      <Typography variant="h5" gutterBottom>Cruce Addi (CSV vs Movimientos)</Typography>
        ...
        <Table size="small">
          <TableHead>
            <TableRow colSpan={6} align="center"  style={{ fontWeight: "bold", backgroundColor: "#CAD420",color: "black", }}>
              <TableCell><b>Email vendedor</b></TableCell>
          ...
          <TableBody>
            {rows.map((r, idx) => (
              <TableRow key={idx}>
                <TableCell>{r.emailVendedor}</TableCell>
