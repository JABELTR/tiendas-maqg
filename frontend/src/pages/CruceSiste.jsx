...
const CruceSiste = () => {
  ...
  const subir = async () => {
    ...
    try {
      ...
      const { data } = await axios.post("/cruce-Siste/upload", form, {
...
const exportarExcel = async () => {
  ...
  try {
    const res = await axios.post("/cruce-Siste/export-excel", formData, {
      headers: { "Content-Type": "multipart/form-data" },
      responseType: "blob",
    });  
    ...
    link.href = url;
    link.setAttribute("download", "cruce_siste.xlsx");
  ...
  return (
    <Box p={2}>
      <Typography variant="h5" gutterBottom>Cruce Sistecrédito (CSV vs Movimientos)</Typography>
        ...
        <Table size="small">
          <TableHead>
            <TableRow colSpan={6} align="center"  style={{ fontWeight: "bold", backgroundColor: "#CAD420",color: "black", }}>
              <TableCell><b>Asesor</b></TableCell>
